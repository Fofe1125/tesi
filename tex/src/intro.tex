...
To give an idea of the analogy with classical fluid mechanics, as previously mentioned, we will give a brief overview of how the GPE can be rearranged to obtain a system of equations that closely resembles the Navier-Stokes equations for an inviscid and incompressible fluid.
...
The quantum fluid is inherently \emph{inviscid}. Unlike the viscous stress tensor $d_{ij}$ \cite{noteZ} present in the Navier-Stokes equations, which is proportional to the velocity gradient and dissipates kinetik energy through friction, while the \emph{quantum stress tensor $\tau_{ij}$} introduces no dissipation. Instead, this term redistrubuate the energy within the system\cite{Tanogami2021}, notably stabilizing the near-zero density structure of the vortex core.
...
Now, we need to determine the function $f(r)$. By imposing the time independency condition, and substituting the above expression into the GPE \eqref{eq:gpe}, we obtain the following boudary value problem:
