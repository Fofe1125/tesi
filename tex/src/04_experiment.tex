...
The first numerical experiment we consider concerns the preservation of the steady-state solution of \eqref{eq:gpe}. Since this solution is stationary, a natural way to assess the quality of our numerical method is to examine how much the approximate solution deviates from the initial one over long integration times.

In all the following experiments, we approximate the density $\rho$ using a 4-th order Padé approximation of the equation \eqref{eq:density}

From now on, the notation $\psi_t$ denotes the numerical solution at time $t > 0$.
...
\begin{figure}[h!]
    ...
    \caption{$\abs{\psi_0}$ (left) and $\abs{\psi_{20}}$ (right), finite differences on a uniform grid.}
\end{figure}

In this figure, we can observe that the numerical solution integrated up to $t = 20$ shows slight variations in the norm outside the vortex core. To highlight this difference more clearly, in Figure \ref{fig:stepper} we employ an alternative colormap with a steeper gradient for values close to $1$
...
We observe that the variations in the norm are localized near the boundaries of the domain, outside the vortex core. To highlight this, Figure~\ref{fig:vortex_center} shows the truncated domain $[-5,5]$, emphasizing the vortex center.
...
We now consider a non-uniformly spaced grid as described in Section \ref{sb:nufg}, with $\delta = h_1 = 1/50$. This results in a grid composed of 347 points per direction.
...
In this zoomed view near the boudary of the domain, we can see that the solution on the non-uniform grid appears to be slightly less accurate, due to the coarser discretization in that region. However, the differences are minimal, and do not significantly affect the overall quality of the solution.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/grid_comp_static.pdf}
    \caption{Grid comparison}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption{Comparison between $\abs{\psi_0}$ (left), $\abs{\psi_{10}}$ (center), and $\abs{\psi_{30}}$ (right), for $x_c = 3$, $m_x = 259$, $m_y = 201$, and number of time steps $n_t = 1500$.}
\end{figure}

With this configuration, the vortices appear to march steadily towards the lateral boundaries of the computational domain. This drift is a direct consequence of the imposed counter-rotation, which breaks the otherwise stationary balance. In Figure \ref{fig:str235} we can appreciate how the translation speed depends strongly on the initial separation between the vortex centers.
...
All simulations reported here were performed on a nonuniform grid along the $x$-direction, reported in \autoref{fig:mvortexgrid} with the spacing defined in Section \ref{sb:nufg}, while in the $y$-direction equispaced nodes were employed.

\pagebreak

\subsection{Simulation of two rotating vortex}
...
\subsection{Comparison between uniform and non-uniform grids}
...
\begin{figure}[h!]
    ...
    \caption{Comparison between equispaced grid and grid as in Figure \ref{fig:rvortexgrid}}
\end{figure}
