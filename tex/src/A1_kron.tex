\chapter{Kronecker product and some properties}
...
\begin{proposition}
    Let $A \in \C^{m \times m}$, $B \in \C^{n \times n}$ and $\uu \in \C^{mn}$. Then 
...
\begin{proof}
    ...
    Since we haven't imposed nothing about $i$ and $k$, this holds for all $i=1,\dots,n$ and $k=1,\dots,m$, so the claim follows.
