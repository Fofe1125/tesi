...
For $B$ we directly compute the exact solution. The treatment for the Laplacian operator is more delicate: since in fact the latter is approximated by the formula \ref{eq:lapprox}, we need to compute the matrix-vector product 
...
This result is way better than the previous one, as now we only have to compute two small matrix exponential and two matrix-matrix products.
